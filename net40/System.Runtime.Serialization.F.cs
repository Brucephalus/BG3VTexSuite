...
namespace System.Runtime.Serialization.Formatters.Soap
{
	...
	internal sealed class SoapHandler
	{
		...
		private void MarshalError(string handler, ParseRecord pr, string value, InternalParseStateE currentState)
		{
			...
			throw new SerializationException(string.Format(CultureInfo.CurrentCulture, SoapUtil.GetResourceString("Serialization_Syntax"), new object[1] { text }));
		}
			...
			case InternalParseStateE.Object:
				...
				else if (string.Compare(urn, "http://schemas.xmlsoap.org/soap/envelope/", StringComparison.OrdinalIgnoreCase) == 0)
				{
					if (string.Compare(name, "Envelope", StringComparison.OrdinalIgnoreCase) == 0)
					{
						if (isEnvelope)
						{
							throw new SerializationException(string.Format(CultureInfo.CurrentCulture, SoapUtil.GetResourceString("Serialization_Parser_Envelope"), new object[1] { prefix + ":" + name }));
						}
						isEnvelope = true;
		...
		private void ProcessAttributes(ParseRecord pr, ParseRecord objectPr)
		{
			...
			if (text3 != null)
			{
				...
				else
				{
					if (!text3.Equals("0"))
					{
						throw new SerializationException(string.Format(CultureInfo.CurrentCulture, SoapUtil.GetResourceString("Serialization_MustUnderstand"), new object[1] { text3 }));
					}
		...
		private void ProcessType(ParseRecord pr, ParseRecord objectPr)
		{
			...
			if (pr.PRtypeXmlKey != null && pr.PRtypeXmlKey.Length > 0 && pr.PRkeyDt != null && pr.PRkeyDt.Length > 0 && assemKeyToInteropAssemblyTable.ContainsKey(pr.PRtypeXmlKey))
			{
				...
				if (pr.PRdtType == null)
				{
					...
					if (pr.PRdtType == null)
					{
						throw new SerializationException(string.Format(CultureInfo.CurrentCulture, SoapUtil.GetResourceString("Serialization_TypeElement"), new object[1] { pr.PRname + " " + pr.PRkeyDt }));
					}
				}
				if (pr.PRdtType == null)
				{
					throw new SerializationException(string.Format(CultureInfo.CurrentCulture, SoapUtil.GetResourceString("Serialization_TypeElement"), new object[1] { pr.PRname + " " + pr.PRkeyDt + ", " + text2 }));
				}
			}
			else if (pr.PRkeyDt != null)
			{
				...
				if (string.Compare(pr.PRkeyDt, "methodSignature", StringComparison.OrdinalIgnoreCase) == 0)
				{
					...
					{
						throw new SerializationException(string.Format(CultureInfo.CurrentCulture, SoapUtil.GetResourceString("Serialization_MethodSignature"), new object[1] { pr.PRvalue }));
					}
				}
		...
		private Type ProcessGetType(string value, string xmlKey, out string assemblyString)
		{
			...
			else if (xmlKey.Equals(xsdKey) || xmlKey.Equals(soapKey))
			{
				...
				else
				{
					...
					{
						throw new SerializationException(string.Format(CultureInfo.CurrentCulture, SoapUtil.GetResourceString("Serialization_Parser_xsd"), new object[1] { value }));
					}
			...
			else
			{
				if (xmlKey == null)
				{
					throw new SerializationException(string.Format(CultureInfo.CurrentCulture, SoapUtil.GetResourceString("Serialization_Parser_xml"), new object[1] { value }));
				}
				...
				{
					throw new SerializationException(string.Format(CultureInfo.CurrentCulture, SoapUtil.GetResourceString("Serialization_Parser_xmlAssembly"), new object[1] { xmlKey + " " + value }));
				}
				...
				if (type == null)
				{
					...
					if (assembly == null)
					{
						throw new SerializationException(string.Format(CultureInfo.CurrentCulture, SoapUtil.GetResourceString("Serialization_Parser_xmlAssembly"), new object[1] { xmlKey + ":" + text2 + " " + value }));
					}
			...
			if (type == null)
			{
				throw new SerializationException(string.Format(CultureInfo.CurrentCulture, SoapUtil.GetResourceString("Serialization_Parser_xmlType"), new object[1] { xmlKey + " " + text + " " + assemblyString }));
			}
			return type;
		}
		...
		private void ProcessArray(ParseRecord pr, int firstIndex, bool IsInterop)
		{
			...
			text = pr.PRkeyDt.Substring(firstIndex);
			if (IsInterop)
			{
				string text2 = (string)assemKeyToInteropAssemblyTable[pr.PRtypeXmlKey];
				pr.PRarrayElementType = objectReader.Bind(text2, pr.PRprimitiveArrayTypeString);
				if (pr.PRarrayElementType == null)
				{
					pr.PRarrayElementType = SoapServices.GetInteropTypeFromXmlType(pr.PRprimitiveArrayTypeString, text2);
				}
				if (pr.PRarrayElementType == null)
				{
					pr.PRarrayElementType = SoapServices.GetInteropTypeFromXmlElement(pr.PRprimitiveArrayTypeString, text2);
				}
				if (pr.PRarrayElementType == null)
				{
					throw new SerializationException(string.Format(CultureInfo.CurrentCulture, SoapUtil.GetResourceString("Serialization_TypeElement"), new object[1] { pr.PRname + " " + pr.PRkeyDt }));
			...
			if (num2 < 1)
			{
				throw new SerializationException(string.Format(CultureInfo.CurrentCulture, SoapUtil.GetResourceString("Serialization_ArrayDimensions"), new object[1] { pr.PRkeyDt }));
			}
			int rank = 0;
			int[] array = null;
			...
			if (!IsInterop || num3 > 1)
			{
				...
				if (pr.PRarrayElementType == null)
				{
					throw new SerializationException(string.Format(CultureInfo.CurrentCulture, SoapUtil.GetResourceString("Serialization_ArrayType"), new object[1] { pr.PRarrayElementType }));
				}
		...
		private int[] ParseArrayDimensions(string dimString, out int rank, out string dimSignature, out InternalArrayTypeE arrayTypeEnum)
		{
			...
			for (int i = 0; i < array.Length; i++)
			{
				...
				else
				{
					...
					{
						throw new SerializationException(string.Format(CultureInfo.CurrentCulture, SoapUtil.GetResourceString("Serialization_ArrayDimensions"), new object[1] { dimString }));
					}
	...
	internal sealed class SerStack
	{
		...
		internal object Next()
		{
			if (next > top)
			{
				throw new SerializationException(string.Format(CultureInfo.CurrentCulture, SoapUtil.GetResourceString("Serialization_StackRange"), new object[1] { stackId }));
			}
			return objects[next++];
		}
	...
	internal sealed class SoapAssemblyInfo
	{
		...
		internal Assembly GetAssembly(ObjectReader objectReader)
		{
			if (assembly == null)
			{
				...
				if (assembly == null)
				{
					throw new SerializationException(string.Format(CultureInfo.CurrentCulture, SoapUtil.GetResourceString("Serialization_AssemblyString"), new object[1] { assemblyString }));
				}
			}
			return assembly;
		}
	...
	internal sealed class Converter
	{
		...
		internal static InternalPrimitiveTypeE ToCode(string value)
		{
			if (value == null)
			{
				throw new ArgumentNullException("serParser", string.Format(CultureInfo.CurrentCulture, SoapUtil.GetResourceString("ArgumentNull_WithParamName"), new object[1] { value }));
			}
		...
		internal static object FromString(string value, InternalPrimitiveTypeE code)
		{
			...
			case InternalPrimitiveTypeE.Boolean:
				...
				case "false":
					return false;
				default:
					throw new SerializationException(string.Format(CultureInfo.CurrentCulture, SoapUtil.GetResourceString("Serialization_typeCoercion"), new object[2] { value, "Boolean" }));
				}
			case InternalPrimitiveTypeE.TimeSpan:
	...
	internal sealed class SoapWriter
	{
		...
		internal void Write(InternalElementTypeE use, string name, AttributeList attrList, string value, bool isNameEscape, bool isValueEscape)
		{
			...
			case InternalElementTypeE.Member:
				...
				break;
			default:
				throw new SerializationException(string.Format(CultureInfo.CurrentCulture, SoapUtil.GetResourceString("Serialization_UseCode"), new object[1] { use.ToString() }));
			}
			EmitLine();
		}
		...
		private string NameTagResolver(NameInfo typeNameInfo, bool isXsiAppended, string arrayItemName)
		{
			...
			case InternalNameSpaceE.UrtUser:
				...
				if (arrayItemName == null)
				{
					...
					{
						throw new SerializationException(string.Format(CultureInfo.CurrentCulture, SoapUtil.GetResourceString("Serialization_Assembly"), new object[1] { typeNameInfo.NIname }));
					}
			...
			case InternalNameSpaceE.CallElement:
				if (typeNameInfo.NIassemId > 0)
				{
					...
					{
						throw new SerializationException(string.Format(CultureInfo.CurrentCulture, SoapUtil.GetResourceString("Serialization_NameSpaceEnum"), new object[1] { typeNameInfo.NInameSpaceEnum }));
					}
	...
	internal sealed class ObjectReader
	{
		...
		internal object Deserialize(HeaderHandler handler, ISerParser serParser)
		{
			if (serParser == null)
			{
				throw new ArgumentNullException("serParser", string.Format(CultureInfo.CurrentCulture, SoapUtil.GetResourceString("ArgumentNull_WithParamName"), new object[1] { serParser }));
			}
		...
		private void CheckSerializable(Type t)
		{
			...
			{
				throw new SerializationException(string.Format(CultureInfo.CurrentCulture, SoapUtil.GetResourceString("Serialization_NonSerType"), new object[2]
				{
					t.FullName,
		...
		internal void Parse(ParseRecord pr)
		{
			...
			case InternalParseTypeE.MemberEnd:
				...
				break;
			default:
				throw new SerializationException(string.Format(CultureInfo.CurrentCulture, SoapUtil.GetResourceString("Serialization_XMLElement"), new object[1] { pr.PRname }));
		...
		{
			throw new SerializationException(string.Format(CultureInfo.CurrentCulture, SoapUtil.GetResourceString("Serialization_ParseError"), new object[1] { onStack.PRname + " " + onStack.PRparseTypeEnum.ToString() + " " + processing.PRname + " " + processing.PRparseTypeEnum }));
		}
		...
		private void CheckSecurity(ParseRecord pr)
		{
			...
			if (pRdtType != null && IsRemoting)
			{
				...
				{
					throw new ArgumentException(string.Format(CultureInfo.CurrentCulture, SoapUtil.GetResourceString("Serialization_MBRAsMBV"), new object[1] { pRdtType.FullName }));
				}
		...
		private void ParseObject(ParseRecord pr)
		{
			...
			{
				throw new SerializationException(string.Format(CultureInfo.CurrentCulture, SoapUtil.GetResourceString("Serialization_TopObjectInstantiate"), new object[1] { pr.PRname }));
			}
			...
			else
			{
				...
				if (pr.PRdtType == null)
				{
					...
					{
						throw new ServerException(string.Format(CultureInfo.CurrentCulture, SoapUtil.GetResourceString("Serialization_SoapFault"), new object[1] { faultString }));
					}
					throw new SerializationException(string.Format(CultureInfo.CurrentCulture, SoapUtil.GetResourceString("Serialization_TypeElement"), new object[1] { pr.PRname }));
				}
			...
			if (pr.PRnewObj == null)
			{
				throw new SerializationException(string.Format(CultureInfo.CurrentCulture, SoapUtil.GetResourceString("Serialization_TopObjectInstantiate"), new object[1] { pr.PRdtType }));
			}
		...
		private void ParseObjectEnd(ParseRecord pr)
		{
			...
			if (parseRecord.PRnewObj == null)
			{
				if (!(parseRecord.PRdtType == Converter.typeofString))
				{
					throw new SerializationException(string.Format(CultureInfo.CurrentCulture, SoapUtil.GetResourceString("Serialization_ObjectMissing"), new object[1] { pr.PRname }));
				}
				if (parseRecord.PRvalue == null)
				{
					parseRecord.PRvalue = string.Empty;
		...
		private void ParseArray(ParseRecord pr)
		{
			...
			throw new SerializationException(string.Format(CultureInfo.CurrentCulture, SoapUtil.GetResourceString("Serialization_ArrayType"), new object[1] { pr.PRarrayTypeEnum.ToString() }));
		}
		...
		private void ParseMember(ParseRecord pr)
		{
			...
			if (pr.PRdtType == null && parseRecord.PRobjectInfo.isTyped)
			{
				...
				if (pr.PRdtType == null)
				{
					throw new SerializationException(string.Format(CultureInfo.CurrentCulture, SoapUtil.GetResourceString("Serialization_TypeResolved"), new object[1] { parseRecord.PRnewObj?.ToString() + " " + pr.PRname }));
				}
		...
		internal long GetId(string keyId)
		{
			if (keyId == null)
			{
				throw new ArgumentNullException("keyId", string.Format(CultureInfo.CurrentCulture, SoapUtil.GetResourceString("ArgumentNull_WithParamName"), new object[1] { "keyId" }));
			}
			if (keyId != inKeyId)
			{
				inKeyId = keyId;
	...
	internal sealed class ObjectWriter
	{
		...
		internal void Serialize(object graph, Header[] inHeaders, SoapWriter serWriter)
		{
			...
			if (serWriter == null)
			{
				throw new ArgumentNullException("serWriter", string.Format(CultureInfo.CurrentCulture, SoapUtil.GetResourceString("ArgumentNull_WithParamName"), new object[1] { "serWriter" }));
			}
			serObjectInfoInit = new SerObjectInfoInit();
		...
		private void ProcessHeaders(long headerId)
		{
			...
			for (int i = 0; i < headers.Length; i++)
			{
				...
				if (headers[i].Name.Equals("__MethodSignature"))
				{
					...
					{
						throw new SerializationException(string.Format(CultureInfo.CurrentCulture, SoapUtil.GetResourceString("Serialization_MethodSignature"), new object[1] { type }));
					}
		...
		private void Write(WriteObjectInfo objectInfo, NameInfo memberNameInfo, NameInfo typeNameInfo)
		{
			...
			if (obj == null)
			{
				throw new ArgumentNullException("objectInfo.obj", string.Format(CultureInfo.CurrentCulture, SoapUtil.GetResourceString("Serialization_ArgumentNull_Obj"), new object[1] { objectInfo.objectType }));
			}
			if (objectInfo.objectType.IsGenericType)
			{
				throw new SerializationException(string.Format(CultureInfo.CurrentCulture, SoapUtil.GetResourceString("Serialization_SoapNoGenericsSupport"), new object[1] { objectInfo.objectType }));
			}
			...
			else
			{
				...
				for (int i = 0; i < outMemberTypes.Length; i++)
				{
					...
					{
						throw new SerializationException(string.Format(CultureInfo.CurrentCulture, SoapUtil.GetResourceString("Serialization_SoapNoGenericsSupport"), new object[1] { outMemberTypes[i] }));
					}
					Type type = null;
		...
		private void Write(WriteObjectInfo objectInfo, NameInfo memberNameInfo, NameInfo typeNameInfo, string[] memberNames, Type[] memberTypes, object[] memberData, WriteObjectInfo[] memberObjectInfos)
		{
			...
			{
				string typeFullName = objectInfo.GetTypeFullName();
		...
		private void WriteMembers(NameInfo memberNameInfo, NameInfo memberTypeNameInfo, object memberData, WriteObjectInfo objectInfo, NameInfo typeNameInfo, WriteObjectInfo memberObjectInfo, bool isAttribute)
		{
			...
			else
			{
				...
				if (isAttribute)
				{
					throw new SerializationException(string.Format(CultureInfo.CurrentCulture, SoapUtil.GetResourceString("Serialization_NonPrimitive_XmlAttribute"), new object[1] { memberNameInfo.NIname }));
				}
		...
		private void WriteArray(WriteObjectInfo objectInfo, NameInfo memberNameInfo, WriteObjectInfo memberObjectInfo)
		{
			...
			{
				throw new SerializationException(string.Format(CultureInfo.CurrentCulture, SoapUtil.GetResourceString("Serialization_SoapNoGenericsSupport"), new object[1] { elementType }));
			}
			...
			{
				string text = null;
				int num5 = nameInfo.NIname.IndexOf('[');
				if (num5 < 0)
				{
					throw new SerializationException(string.Format(CultureInfo.CurrentCulture, SoapUtil.GetResourceString("Serialization_Dimensions"), new object[1] { arrayElemTypeNameInfo.NIname }));
				}
				text = nameInfo.NIname.Substring(num5);
				nameInfo.NIname = arrayElemTypeNameInfo.NIname + "[" + array2[0] + "]";
				nameInfo.NIobjectId = objectId;
				...
				for (int num6 = array3[0]; num6 < array4[0] + 1; num6++)
				{
					WriteArrayMember(objectInfo, arrayElemTypeNameInfo, array6[num6]);
				}
				break;
			}
			default:
			{
				int num = nameInfo.NIname.IndexOf('[');
				StringBuilder stringBuilder = new StringBuilder(10);
		...
		private object GetNext(out long objID)
		{
			...
			if (firstTime)
			{
				throw new SerializationException(string.Format(CultureInfo.CurrentCulture, SoapUtil.GetResourceString("Serialization_ObjNoID"), new object[1] { obj }));
			}
			return obj;
		}
	...
	internal sealed class WriteObjectInfo
	{
		...
		internal void InitSerialize(object obj, ISurrogateSelector surrogateSelector, StreamingContext context, SerObjectInfoInit serObjectInfoInit, IFormatterConverter converter, SoapAttributeInfo attributeInfo, ObjectWriter objectWriter)
		{
			...
			else if (obj is ISerializable)
			{
				if (!objectType.IsSerializable)
				{
					...
					{
						objectType.FullName,
						objectType.Module.Assembly.FullName
					}));
				}
				si = new SerializationInfo(objectType, converter);
		...
		internal Type GetMemberType(MemberInfo objMember)
		{
			...
			throw new SerializationException(string.Format(CultureInfo.CurrentCulture, SoapUtil.GetResourceString("Serialization_SerMemberInfo"), new object[1] { objMember.GetType() }));
		}
		...
		private int Position(string name)
		{
			...
			throw new SerializationException(string.Format(CultureInfo.CurrentCulture, SoapUtil.GetResourceString("Serialization_Position"), new object[1] { objectType?.ToString() + " " + name }));
		}
	...
	internal sealed class ReadObjectInfo
	{
		...
		internal MemberInfo GetMemberInfo(string name)
		{
			if (isSi)
			{
				throw new SerializationException(string.Format(CultureInfo.CurrentCulture, SoapUtil.GetResourceString("Serialization_MemberInfo"), new object[1] { objectType?.ToString() + " " + name }));
			}
			if (cache.memberInfos == null)
			{
				throw new SerializationException(string.Format(CultureInfo.CurrentCulture, SoapUtil.GetResourceString("Serialization_NoMemberInfo"), new object[1] { objectType?.ToString() + " " + name }));
			}
		...
		internal Type GetType(string name)
		{
			...
			if (type == null)
			{
				throw new SerializationException(string.Format(CultureInfo.CurrentCulture, SoapUtil.GetResourceString("Serialization_ISerializableTypes"), new object[1] { objectType?.ToString() + " " + name }));
			}
			return type;
		}
		...
		internal Type GetType(int position)
		{
			...
			if (isTyped)
			{
				...
				{
					throw new SerializationException(string.Format(CultureInfo.CurrentCulture, SoapUtil.GetResourceString("Serialization_ISerializableTypes"), new object[1] { objectType?.ToString() + " " + position }));
				}
